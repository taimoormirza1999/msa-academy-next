...
const VideoGallery = () => {
  ...
  return (
    <div className="mx-auto px-2 w-[96%] lg:pe-10 mt-28 lg:-mt-10">
      <div className="grid grid-cols-2 lg:grid-cols-4 grid-flow-row mt-10 gap-3 gap-x-6 md:gap-y-24 lg:gap-7">
        {videos.map((video, index) => {
          ...
          return (
            <motion.div
              key={index}
              {...motionProps}
              className={`h-54 lg:h-48 flex justify-center items-center text-center rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300 ${
                index === 0 || index === 2 ? '-mt-28 lg:-mt-0' : '-mt-16 lg:-mt-0'
              }`}
            >
